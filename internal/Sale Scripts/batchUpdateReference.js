...
$(document).on('knack-view-render.view_395', function(event, view) {
  ...
  $('<button id="update"">Top Shelf These Fools</button>').insertAfter(
      '.view-header'
  );
  ...
  $('#update').click(function() {
    ...
    let selectedRecords = record_ids.length + 1;
    function updateRecords(id, records, data) {
      $.ajax({
        // CHANGE OBJECT_ID TO YOUR OWN OBJECT ID
        url: 'https://api.knackhq.com/v1/objects/object_4/records/' + id,
        type: 'PUT',
